            ...
            foreach ($allUpUsers as $item_uid){
                ...
                if ($balance['balance']>=200){
                    if(!$is_grade){
                        ...
                        model('UserVip')->where('uid',$item_uid)->update(['grade'=>1,'name'=>50]);
                    }
                }elseif ( //v2
                    $balance['balance']>=2000
                    && $allSubMembers[$item_uid]['one']>=3
                    ){
                    model('UserVip')->where('uid',$item_uid)->update(['grade'=>2,'name'=>55]);
                }elseif (  //v3
                    $balance['balance']>=5000
                    && $allSubMembers[$item_uid]['one']>=10
                    && $allSubMembers[$item_uid]['two']>=5){
                    model('UserVip')->where('uid',$item_uid)->update(['grade'=>3,'name'=>60]);
                }elseif ( //v4
                    $balance['balance']>=10000
                    && $allSubMembers[$item_uid]['one']>=30
                    && $allSubMembers[$item_uid]['two']>=20
                    && $allSubMembers[$item_uid]['three']>=10){
                    model('UserVip')->where('uid',$item_uid)->update(['grade'=>4,'name'=>65]);
                }elseif ( //v5
                    ...
                    model('UserVip')->where('uid',$item_uid)->update(['grade'=>5,'name'=>70]);
                }elseif ( //v6
                    $balance['balance']>=50000
                    && $allSubMembers[$item_uid]['one']>=100
                    && $allSubMembers[$item_uid]['two']>=50
                    && $allSubMembers[$item_uid]['three']>=30){
                    model('UserVip')->where('uid',$item_uid)->update(['grade'=>6,'name'=>75]);
                }else{
                    $rid = model('UserVip')->insert($updateData);
                }
            }
